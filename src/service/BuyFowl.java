...
for (int numberOfCocks=1;numberOfCocks<maxOfCocks;numberOfCocks++){
    for (int numberOfHen=1;numberOfHen<maxOfHen;numberOfHen++){
        ...
        boolean totalPrice =numberOfCocks*cock.getPrice()+numberOfHen*hen.price+numberOfChick/3 ==price;
        if (multipleOfThree && totalPrice){
            System.out.println("买公鸡"+numberOfCocks+"只,买母鸡"+numberOfHen+"只，买小鸡"+numberOfChick+"只");
            System.out.println("----------------------------------------------");
        }
